...
export async function enableReminders(time='22:00'){
 ...
 const permissions=await LocalNotifications.requestPermissions();if(permissions.display!=='granted')throw new Error('通知权限未开启，请在手机设置中允许小松工作台发送通知');
 ...
 await LocalNotifications.schedule({notifications:[{id:1,title:'小松工作台 · 给今天一个收尾',body:'回顾今天，安排明天最重要的事。',channelId:'action-reminders',schedule:{on:{hour,minute},repeats:true,allowWhileIdle:true},extra:{route:'summary'}}]});
